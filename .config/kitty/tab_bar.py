...
bat_text_color = as_rgb(0x7FBBB3)
...
def get_battery_status():
    try:
        result = subprocess.run(
            ["acpi", "-b"], capture_output=True, text=True, check=True
        )
        output = result.stdout.strip()
        status, percentage = None, None
        if output:
            parts = output.split(", ")
...
def get_battery_icon(percentage, is_charging):
    if is_charging:
        return PLUGGED_ICON
    return UNPLUGGED_ICONS.get((percentage // 10) * 10, UNPLUGGED_ICONS[100])
...
def _draw_right_status(screen: Screen, is_last: bool) -> int:
    ...
    battery_icon = (
        get_battery_icon(percentage, status == "Charging")
    ...
    cells = []
    if battery_icon:
        cells.append((bat_text_color, f" {battery_icon} {percentage}% "))
