...
DEFAULT_SCHEME: str = SCHEMES[2]

# Colorfulness thresholds
LOW_COLORFULNESS: int | float = 10
MEDIUM_COLORFULNESS: int | float = 20
HIGH_COLORFULNESS: int | float = 40
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Pick a color scheme based on image colorfulness."
    )
    ...
    if not args.image.is_file():
        print(DEFAULT_SCHEME, file=sys.stderr)
        sys.exit(1)
